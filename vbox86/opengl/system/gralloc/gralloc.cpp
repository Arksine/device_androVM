                         ...
                         buffer_handle_t* pHandle, int* pStride)
{
    ...
    if (!grdev || !pHandle || !pStride) {
        LOGE("gralloc_alloc: Bad inputs (grdev: %p, pHandle: %p, pStride: %p",
    ...
    if (hw_write && sw_write) {
        LOGE("gralloc_alloc: Mismatched usage flags: %d x %d, usage %x",
                w, h, usage);
        return -EINVAL;
    }
    ...
    if (format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED) {
        ...
        if (format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED) {
            LOGE("gralloc_alloc: Requested auto format selection, "
    ...
    switch (format) {
        ...
        case HAL_PIXEL_FORMAT_YV12:
            ...
            break;
        default:
            LOGE("gralloc_alloc: Unknown format %d", format);
            return -EINVAL;
    }
...
static int fb_manageOrientation(struct framebuffer_device_t* dev, int orientation)
{
    ...
    LOGI("setOrientation: orientation=%d\n", orientation);
...
static void fb_setOrientation(struct framebuffer_device_t* dev, int orientation)
{
    if (fb_manageOrientation(dev, orientation)<0)
        LOGE("Error setting Orientation");
}
